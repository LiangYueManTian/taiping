 ...
 */
@Component
public class PduAnalyze extends AbstractCapacityAnalyze {
     ...
     * @param capacityPduStatisticsDto 电力容量pdu的参数
     * @return 比较的值
     */
    public static String getCompareValue(CapacityPduStatisticsDto capacityPduStatisticsDto) {
        return capacityPduStatisticsDto.getCabinetUniqueName();
    }
     ...
     * @return 总数的值
     */
    public static double getAllValue(CapacityPduStatisticsDto capacityPduStatisticsDto) {
        return capacityPduStatisticsDto.getRatedPower();
    }
     ...
     * @param pduStatisticsDtoList 一年的数据
     * @return 返回同比
     */
    protected List<CapacityThresholdInfo> calculateYearOverYear(Integer analysisYear, Integer analysisMonth,
                                                                ...
                                                                List<CapacityPduStatisticsDto> pduStatisticsDtoList) {
        //计算同比
        if (!ObjectUtils.isEmpty(capacityThresholdInfoList)) {
            ...
            String type = "1";
            capacityThresholdInfoList = electricAnalyze.castGroupInfo(capacityThresholdInfoList, pduStatisticsDtoList, beforeYearDate, type, ThresholdTypeEnum.PDU);
        }
    ...
    protected List<CapacityThresholdInfo> calculateRingGrowth(Integer analysisYear, Integer analysisMonth,
        ...
        if (!ObjectUtils.isEmpty(capacityThresholdInfoList)) {
            ...
            String type = "2";
            capacityThresholdInfoList = electricAnalyze.castGroupInfo(capacityThresholdInfoList, pduStatisticsDtoList, beforeDate, type, ThresholdTypeEnum.PDU);
        }
        return capacityThresholdInfoList;
    }
    ...
    protected CapacityAnalyzeData analyzeDataRealize(Map<String, List<SystemSetting>> searchCapacityThresholdList, Integer analysisYear, Integer analysisMonth) {
        //获取pdu的阈值信息
        List<SystemSetting> systemSettingList  = searchCapacityThresholdList.get(ThresholdCapacityEnum.PDU_ELECTRIC_PERCENT.getThresholdCode());

        //查询一年每个pdu的电力容量占比
        List<CapacityPduStatisticsDto> pduStatisticsDtoList = this.getSearchPduStatisticsDtoList(analysisYear, analysisMonth);

        //算出每一个pdu的空间和电力容量占比
        pduStatisticsDtoList = this.setPercentToData(pduStatisticsDtoList);

        //当月数据
        List<CapacityPduStatisticsDto> nowMonthList = pduStatisticsDtoList.stream().
                filter(c -> c.getMonth().equals(analysisMonth) && c.getYear().equals(analysisYear)).collect(Collectors.toList());

        //容量楼层阈值信息
        List<CapacityThresholdInfo> capacityThresholdInfoList = electricAnalyze.getThresholdList(nowMonthList, analysisMonth, analysisYear, systemSettingList, ThresholdTypeEnum.PDU);
        //计算环比
        capacityThresholdInfoList = calculateRingGrowth(analysisYear, analysisMonth, capacityThresholdInfoList, pduStatisticsDtoList);
        //计算同比
        capacityThresholdInfoList = calculateYearOverYear(analysisYear, analysisMonth, capacityThresholdInfoList, pduStatisticsDtoList);
        //计算推荐或显示数据
        List<CapacityThresholdRelatedInfo> relatedInfoList = new ArrayList<>();
        //计算图表显示信息
        List<CapacityThresholdRelatedViewInfo> viewInfoList = electricAnalyze.calculateViewInfo(analysisYear, analysisMonth, capacityThresholdInfoList, pduStatisticsDtoList, ThresholdTypeEnum.PDU);
        //形成建议信息
        capacityThresholdInfoList = CapacityAnalyze.setAdviceToThreshold(capacityThresholdInfoList, relatedInfoList, ThresholdTypeEnum.PDU);
        //形成运维管理活动项
        List<ManageActivity> manageActivityList = capacityAnalyze.generateManageActivity(capacityThresholdInfoList, ThresholdTypeEnum.PDU);
     ...
     * @param pduDtoList pdu集合
     * @return 返回统计数据
     */
    public List<CapacityPduStatisticsDto> setPercentToData(List<CapacityPduStatisticsDto> pduDtoList) {
        if (!ObjectUtils.isEmpty(pduDtoList)) {
            for (CapacityPduStatisticsDto capacityPduStatisticsDto : pduDtoList) {
                double usedPower = capacityPduStatisticsDto.getUsedActualPower();
                double allRatedPower = capacityPduStatisticsDto.getRatedPower();
                //计算pdu电力容量占比
                double powerPercent = CalculateUtil.castPercent(usedPower, allRatedPower);
                capacityPduStatisticsDto.setUsedActualPercent(powerPercent);
            }
        }
        return pduDtoList;
    }
     ...
     */
    public List<CapacityPduStatisticsDto> getSearchPduStatisticsDtoList(Integer analysisYear, Integer analysisMonth) {
        CabinetInfoListParameter parameter = ElectricAnalyze.generateSearchParam(analysisYear, analysisMonth);
        List<CapacityPduStatisticsDto> pduStatisticsDtoList = cabinetService.queryCapacityByPdu(parameter);
        return pduStatisticsDtoList;
    }
}
